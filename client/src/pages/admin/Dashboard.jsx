...
export default function Dashboard() {
  ...
  const handleDelete = async (agreement) => {
    if (window.confirm(`Are you sure you want to delete ${agreement.file_name}?`)) {
      try {
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <form onSubmit={handleUpload} className="mb-6">
          ...
          <div className="mt-1 flex items-center">
            ...
            />
            <button type="submit" disabled={uploading} className="ml-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400">
          ...
          {agreements.length > 0 ? (
            <ul className="divide-y divide-gray-200">
              {agreements.map(agreement => (
                <li key={agreement.id} className="py-3 flex items-center justify-between">
                  <span className="text-gray-800">{agreement.file_name}</span>
                  <div className="space-x-2">
                    <button onClick={() => handleDownload(agreement.storage_path)} className="text-sm text-blue-600 hover:underline">Download</button>
                    <button onClick={() => handleDelete(agreement)} className="text-sm text-red-600 hover:underline">Remove</button>
                  </div>
                </li>
              ))}
            </ul>
